...
def index():
    ...
    print(friends)
    return render_template("index.html", friends = friends)# the = friends here is the same friends variable from a few lines above that received all the information returned from the friend.py, sends the all_friends to the HTML.
...
def one_friend(friend_id):
    ...
    one_friend = Friend.get_one_friend(query_data) # one_friend storing the singular instance we created, now stored in one friend
